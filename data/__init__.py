...
from .all_code_benchmarks import CodeData
from .caption_benchmarks import CaptionData
from .summarization_benchmarks import SummarizationData
...
def load_task(task_name: str, evaluation_api: EngineLM, *args, **kwargs) -> Tuple[Dataset, Dataset, Callable]:
    ...
    if "object_counting" in task_name:
        ...
        eval_fn = StringBasedFunction(string_based_equality_fn, function_purpose=fn_purpose)
        return train_set, val_set, test_set, eval_fn
    elif task_name.lower() in ['flickr8k', 'llava-critic']:

        train_set = CaptionData(task_name, split = 'train')
        val_set = CaptionData(task_name, split = 'val')
        test_set = CaptionData(task_name, split = 'test')
        eval_fn = ''
        return train_set, val_set, test_set, eval_fn
    elif task_name.lower() in ['cnndaily', 'xsum']:
        print('loading', task_name, 'for summarization')
        train_set = SummarizationData(task_name, split = 'train')
        val_set = ''#SummarizationData(task_name, split = 'val')
        test_set = SummarizationData(task_name, split = 'test')
        eval_fn = ''
        return train_set, val_set, test_set, eval_fn
    else:
...
def load_instance_task(task_name: str, evaluation_api: EngineLM,  *args, **kwargs):
    if "MMLU_" in task_name:
        ...
        return test_set
    elif task_name.lower() in ['leetcode', 'humaneval','evoeval_combine', 'evoeval_creative', 'evoeval_difficult', 'evoeval_subtle', 'evoeval_tool', 'core_eval']:
        return CodeData(task_name)
