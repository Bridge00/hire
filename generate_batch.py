...
def is_together_model(model_name):
    # Check if it's in our mapping or starts with known Together prefixes
    full_name = resolve_model(model_name)
    together_prefixes = ["Qwen/", "meta-llama/", "mistralai/", "deepseek-ai/"]
    return any(full_name.startswith(p) for p in together_prefixes) or "/" in full_name
...
def generate_code_batch(args, dataset, cache_dir):
    ...
    with open(output_file, 'w', encoding='utf-8') as f_out:
        for i in range(start, end):
            try:
                task_id, prompt, _, _, _ = dataset[i]
        ...
        for i in range(start, end):
            try:
                ...
                if args.eval_prompt == "vanilla":
                    ...
                    expl_path = os.path.join(cache_dir, args.dataset, model_or_source, source_model, source_folder, f"{task_id}.json")
                    ...
                    source_model = args.explainer_model or args.eval_model
                    pseudo_path = os.path.join(cache_dir, args.dataset, model_or_source, source_model, source_folder, f"{task_id}.json")
                    ...
                    source_model = args.explainer_model or args.eval_model
                    explainer_path = os.path.join(cache_dir, args.dataset, model_or_source, source_model, source_folder, f"{task_id}.json")
                    ...
                    # We might need to also include the evaluator model if it's an update variant
                    source_folder = source_prompt + suffix + update_suffix
                    if args.lambda_val is not None:
                         source_folder = f"{source_prompt}{suffix}_lambda_L{args.lambda_val}{update_suffix}"
                    ...
                    checker_prompt = up.HIRE_EXPLAINER_ALIGNMENT_CHECKER_FAITHFUL if is_faithful else up.HIRE_EXPLAINER_ALIGNMENT_CHECKER
                ...
                # HIRE Family Logic
                    ...
                    # Downstream Checkers (Plan, Impl)
                        ...
                        source_model = args.explainer_model or args.eval_model
                        decomposed_plan_path = os.path.join(cache_dir, args.dataset, model_or_source, source_model, source_decomposer_base, f"{task_id}.json")
                        ...
                        # AGGREGATOR
                             ...
                             source_model = args.explainer_model or args.eval_model
                             plan_path = os.path.join(cache_dir, args.dataset, model_or_source, source_model, plan_folder, f"{task_id}.json")
                             ...
                             commentor_path = os.path.join(cache_dir, args.dataset, model_or_source, source_model, commentor_folder, f"{task_id}.json")
                        ...
                        # IMPLEMENTATION CHECKERS
                             ...
                             for idx, step in enumerate(steps):
                                ...
                                source_model = args.explainer_model or args.eval_model
                                step_cache_path = os.path.join(cache_dir, args.dataset, model_or_source, source_model, step_folder, f"{task_id}.json")
                ...
                if args.eval_source:
                        ...
                        eval_cache_path = os.path.join(cache_dir, args.dataset, args.eval_source, model_name, cache_prompt_type, f"{task_id}.json")
                else:
                        eval_cache_path = os.path.join(cache_dir, args.dataset, args.code_gen_model, model_name, cache_prompt_type, f"{task_id}.json")
...
def generate_reconstruct_batch(args, dataset, cache_dir):
    ...
    reconstruct_model = args.reconstruct_model or args.code_gen_model or "gpt-4o-mini"
    ...
    with open(output_file, 'w', encoding='utf-8') as f_out:
        for i in range(start, end):
            try:
                ...
                reconstruct_path = os.path.join(cache_dir, args.dataset, model_or_source, reproduce_model_folder, reconstruct_prompt, f"{task_id}.json")
                ...
                model_name = resolve_model(args.eval_model)
                is_together = is_together_model(model_name)
    ...
    with open(output_file, 'w', encoding='utf-8') as f_out:
        for i in range(start, end):
            try:
                ...
                feedback_path = os.path.join(cache_dir, args.dataset, model_or_source, source_model, compare_prompt_type, f"{task_id}.json")
...
def generate_self_refine_batch(args, dataset, cache_dir):
    ...
    l_suffix = f"_L{args.lambda_val}" if args.lambda_val is not None else ""
    model_name_part = args.eval_model
...
def generate_refine_batch(args, dataset, cache_dir):
    ...
    n_suffix = f"_N_{args.n}" if (args.eval_prompt.startswith("hire_") and args.eval_prompt not in ["hire_explainer", "hire_explainer_query_aware", "hire_explainer_checker", "hire_explainer_checker_query_aware", "hire_explainer_alignment_checker", "hire_explainer_alignment_checker_query_aware", "hire_explainer_alignment_checker_lambda", "hire_explainer_alignment_checker_query_aware_lambda", "hire_explainer_lambda", "hire_explainer_query_aware_lambda", "hire_explainer_checker_lambda", "hire_explainer_checker_query_aware_lambda", "hire_pseudo", "hire_pseudo_query_aware", "hire_pseudo_checker", "hire_pseudo_checker_query_aware"]) else ""
    l_suffix = f"_L{args.lambda_val}" if args.lambda_val is not None else ""
    model_name_part = f"{args.explainer_model}_{args.eval_model}" if args.explainer_model else args.eval_model
            ...
            try:
                ...
                if args.eval_prompt == "vanilla" and args.k > 1:
                    ...
                    for k_idx in range(1, args.k + 1):
                        ...
                        eval_cache_path = os.path.join(cache_dir, args.dataset, args.eval_source, model_folder, eval_prompt_type, f"{task_id}.json")
                    ...
                    if args.lambda_val is not None:
                        if "_lambda" not in eval_prompt_type:
                            ...
                            eval_prompt_type = f"{args.eval_prompt}_L{args.lambda_val}"
                    elif args.eval_prompt.startswith("hire_") and args.eval_prompt not in ["hire_explainer", "hire_explainer_query_aware", "hire_explainer_checker", "hire_explainer_checker_query_aware"]:
...
def main():
    ...
    parser.add_argument("--mode", type=str, required=True, choices=["code", "eval", "refine", "reconstruct", "compare", "update", "direct_update", "self_refine"], 
                        help="Mode: 'code' for code generation, 'eval' for evaluation, 'refine' for code refinement, 'reconstruct' for code reproduction from explanations, 'compare' for explanation feedback, 'update' for explanation improvement, 'direct_update' for direct explanation improvement skipping feedback, 'self_refine' for explanation refinement by self-reflection")
    ...
    parser.add_argument("--reconstruct_model", type=str, help="Model for reconstruction (defaults to code_gen_model if mode=reconstruct)")
    
    # Submission control
    parser.add_argument("--dont_submit", action="store_true", help="Don't submit batch to OpenAI (only generate the file)")
    parser.add_argument("--eval_source", type=str, nargs='+', help="Key(s) in dataset to evaluate instead of generated code (e.g. 'canonical_solution')")
